						...
						<tr>
							...
							<td class="p-0">
								<table class="table table-borderless w-auto mb-0">
									<tr>
										<td><input class="form-control form-control-sm w-auto" type="text" v-model="aditional_data.sourceID" size="20"></td>
...
<script>
export default {
	props: ['citationArgs', 'aditional_data'],
	data() {
		return {
			cid: '',
			citation: {
	...
	methods: {
		add() {
			...
			this.$store.dispatch('citation/add', {
				...this.citation,
				...this.citationArgs,
				...this.aditional_data
			}).then(response => {
				...
				this.$emit('citationAdded', response.data);
			}).catch(error => {});
		},
		addSource() {
			this.$emit('addSource');
