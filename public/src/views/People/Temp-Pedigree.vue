<template>
	<div class="pedigree-container">
		<p v-if="ptype == 'pedigree standard' || ptype == 'pedigree box'"> (Notes: You may have to scroll down or right to see chart. <span class="arro2"> </span> Additional information New pedigree) </p>
		<p v-if="ptype == 'pedigree compact'"> Notes: You may have to scroll down or right to see chart.) </p>
			...
			<panZoom selector="#g3" :options="{minZoom: 1, maxZoom: 1}">
				<div>
					<ul id="g3" v-if="people">
						<li>
							<div class="persion-ind">
								<router-link class="tp-name" :to="{name: 'people-single', params:{id: people ? people.id : ''}}">{{people ? people.name : ''}} {{people.personID}} </router-link>
								<span class="tp-sex" v-if="people.sex=='M'">♂</span>
								<span class="tp-sex" v-if="people.sex=='F'">♀</span>
								<div class="date">
									{{people.birthdate || people.deathdate ? '(' : ''}}
												...
												<ul class="hover-lul">\
													<li> <strong>Children: </strong>\
														<ul class="hover-lul" v-if="family.childrens.length">\
															<li v-for="(child, indexchild) in family.childrens"> {{indexchild+1}}. \
